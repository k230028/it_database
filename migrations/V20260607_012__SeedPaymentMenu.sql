...
--   - 신규 LNK: MINF0026 "대금지급" → /project/payment (depth 3, SNO 110)
--   - 권한: CMENUA 행 없음 → 전체 인증 사용자 접근 (입찰/계약/MINF0025와 동일 정책)
SET DEFINE OFF;
...
USING (
  SELECT '/project/payment' AS SRE_PTH,
         '대금지급'          AS SRE_MNU_NM,
...
-- =====================================================================
-- 2) 메뉴 트리
--    MINF0026: LNK, depth 3, MINF0015(정보화사업 GRP) 하위, SNO 110
--    WHL_MNU_PTH: /MHED0002/MINF0015/MINF0026
-- =====================================================================
MERGE INTO TPRMPP_CMENUM t
USING (
  SELECT 'MINF0026'                    AS MNU_ID,
         'MINF0015'                    AS HRK_MNU_ID,
         '대금지급'                    AS MNU_NM,
         'LNK'                         AS MNU_TP_C,
         '/project/payment'            AS SRE_PTH,
         110                           AS MNU_SOT_SQN_SNO,
         'N'                           AS HID_YN,
         3                             AS MNU_DEP,
         '/MHED0002/MINF0015/MINF0026' AS WHL_MNU_PTH
  FROM DUAL
...
-- =====================================================================
-- 3) 권한 매핑: 없음.
--    CMENUA 행 없음 = 전체 인증 사용자 접근 (입찰/계약/MINF0025와 동일).
-- =====================================================================
